...
#Region "QuotationLine Variable "
        ...
        Private mvarLineRemarks As String
        '****
#End Region
...
#Region " Quotation Header SELECT Statement "
        Public Function GetQuotationHeader(ByRef ds As DataSet, Optional ByVal pQuotation_ID As String = "", Optional ByVal pStatus As String = "") As Boolean
            ...
            SQLStr = SQLStr & " AND ((Archived IS NULL OR Archived = '0') "
            SQLStr = SQLStr & " OR (Trash IS NULL OR Trash = '0')) "
            SQLStr = SQLStr & " ORDER BY QH.Create_Date DESC"
        ...
        Public Function GetQuotationHeaderWORevisedQuo(ByRef ds As DataSet) As Boolean
            ...
            SQLStr = SQLStr & " AND ((Archived IS NULL OR Archived = '0') "
            SQLStr = SQLStr & " OR (Trash IS NULL OR Trash = '0')) "
            SQLStr = SQLStr & " ORDER BY Quotation_NO DESC"
                ...
                If .GetDataSet(ds, SQLStr) Then
                    If ds.Tables(0).Rows.Count > 0 Then
                        If Not IsDBNull(ds.Tables(0).Rows(0)("ItemNo")) Then
                            mvarItemNo = ds.Tables(0).Rows(0)("ItemNo")
                        Else
                            mvarItemNo = ""
                        End If
                    Else
...
#Region " Quotation Line SELECT Statement "
        Public Function GetQuotationLine(ByRef ds As DataSet)
            ...
            SQlStr = SQlStr & " Quotation_ID = " & mvarQuotation_ID
            SQlStr = SQlStr & " Order by ItemNo ASC"
...
#Region "Copy QuotationLine to Clipboard"
        Public Function ClearClipboard_QuotationLine(ByVal pUserId As String) As Boolean
            Dim ds As DataSet
            Dim SQLStr As String

            SQLStr = "Delete from itemClipboard where type = 'QuotationLine' and userid=" & pUserId
            With New DataAccess.DAccess.Access
                If .GetDataSet(ds, SQLStr) Then
                    Return True
                End If
        ...
        Public Function CopyClipboard_QuotationLine(ByVal pQuotation_Line_Id As Integer, ByVal pOrderno As Integer, ByVal pUserId As String) As Boolean
            Dim SQLStr As String
            Dim ds As DataSet

            SQLStr = "insert into itemClipboard (ID,Orderno,Type,UserID) values (" & pQuotation_Line_Id & " ," & pOrderno & ",'QuotationLine', " & pUserId & ");"
            With New DataAccess.DAccess.Access
                If .GetDataSet(ds, SQLStr) Then
                    Return True
                End If
        ...
        Public Function IsClipboard_QuotationLine_Empty(ByVal pUserId As String) As Boolean
            Dim ds As DataSet
            Dim SQLStr As String

            SQLStr = "select * from itemClipboard where type = 'QuotationLine' and userid=" & pUserId
            With New DataAccess.DAccess.Access
                If .GetDataSet(ds, SQLStr) Then
        ...
        Public Function GetClipboard_QuotationLine_Empty(ByRef ds As DataSet, ByVal pUserId As String) As Boolean

            Dim SQLStr As String

            SQLStr = "select * from itemClipboard where type = 'QuotationLine' and userid=" & pUserId & " order by orderno  asc"
            With New DataAccess.DAccess.Access
                If .GetDataSet(ds, SQLStr) Then
                    Return True
                End If
...
#Region " Quotation Line SAVE Statement "
        Public Function SaveQuotationLineFirstTime() As Boolean
            ...
            SQLStr = SQLStr & "(Quotation_ID,ItemNo,BoxCategory_ID,Qty,Create_Date,Transact_Date,UserID)"
            SQLStr = SQLStr & " VALUES (" & mvarQuotation_ID & "," & mvarItemNo & "," & mvarBoxCategory_ID & "," & mvarQty & ","
...
#Region " Quotation Line UPDATE SQL Statement "
        Public Function UpdateQuotationLine()
            ...
            If Len(mvarLineRemarks) > 0 Then
                SQLStr = SQLStr & " Remarks ='" & mvarLineRemarks & "',"
            Else
                SQLStr = SQLStr & " Remarks = NULL,"
            End If
...
#Region " Quotation Line Component SELECT Statement "
        ...
        Public Function GetQuotationLineComp(ByRef ds As DataSet) As Boolean
            ...
            SQLStr = SQLStr & " FROM QuotationLineComponent QLC left JOIN "
...
#Region " Quotation Line Component UPDATE Statement "
        Public Function UpdateQuotationLineComponent()
            ...
            SQLstr = SQLstr & "Adhoc_Description = '" & mvarAdhoc_Description & "',"
            SQLstr = SQLstr & "Transact_Date = GetDate(),"
...
#Region " Quotation Delete Statement "
        ...
        Public Function DeleteQuotationLine(Optional ByVal pReAdjustitemno As Boolean = True) As Boolean
            Dim ds As DataSet
            With New DataAccess.DAccess.Access
                Dim SQLstr As String = "DELETE FROM QuotationLine WHERE Quotation_Line_ID = " & mvarQuotation_Line_ID
                If pReAdjustitemno = True Then
                    SQLstr = SQLstr & ";Exec ReAdjustQuotationLineItemno " & mvarQuotation_ID & "," & mvarItemNo
                End If
                SQLstr = SQLstr & ";"
                If .GetDataSet(ds, SQLstr) Then
                    Return True
        ...
        Public Function ReAdjustQuotationLineItemNo() As Boolean
            Dim ds As DataSet
            Dim SQLstr As String
            With New DataAccess.DAccess.Access
                SQLstr = "Exec ReAdjustQuotationLineItemno " & mvarQuotation_ID & ",0"
                If .GetDataSet(ds, SQLstr) Then
                    Return True
                End If
            End With
...
#Region " Quotation Report "
        Public Function GetQuotationRpt(ByRef ds As DataSet, ByVal pQuotation_ID As String, _
            ...
            SQLStr = SQLStr & ", b.Remarks as LineRemarks "
            ...
            If noLine = False Then
                ...
                SQLStr = SQLStr & " (SELECT b.Qty * b.Cost AS Expr4, a.Quotation_id "
            ...
            'If noLine = False Then
            SQLStr = SQLStr & " left join QuotationLine b on (a.Quotation_ID = b.Quotation_ID) "
            'End If
            'If nocomp = False Then
            SQLStr = SQLStr & " left join QuotationLineComponent c on (b.Quotation_Line_ID = c.Quotation_Line_ID) "
            ...
            SQLStr = SQLStr & "left join PaymentDetails i on (a.Payment_Term = i.PayDetails_ID)"
            SQLStr = SQLStr & "left join DeliveryTerms j on (a.Delivery_Term =j.DelTerms_ID)"
...
#Region " Order Confirmation Report "
        Public Function GetOrderConfirmationRpt(ByRef ds As DataSet, ByVal pQuotation_ID As String, _
            ...
            SQLStr = SQLStr & " left join QuotationLine b on b.Quotation_ID = a.Quotation_ID "
            SQLStr = SQLStr & " left join QuotationLineComponent c on (b.Quotation_Line_ID=c.Quotation_Line_ID) "
            SQLStr = SQLStr & " left join Product f on (c.product_id=f.product_id) "
            SQLStr = SQLStr & " left join PriceInclude g on (a.PriceInclude_ID = g.PriceInclude_ID) "
...
#Region " QuotationLine's Get n Let Property "
        ...
        Public Property LineRemarks() As String
            Get
                Return mvarLineRemarks
            End Get
            Set(ByVal Value As String)
                mvarLineRemarks = Value
            End Set
        End Property
#End Region
